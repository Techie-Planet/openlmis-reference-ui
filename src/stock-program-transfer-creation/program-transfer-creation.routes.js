...
(function() {
    ...
    function routes($stateProvider, STOCKMANAGEMENT_RIGHTS, SEARCH_OPTIONS, ADJUSTMENT_TYPE) {
        $stateProvider.state('openlmis.stockmanagement.programtransfer.creation', {
            isOffline: true,
            url: '/:programId/:programToId/create?page&size&keyword',
            views: {
                '@openlmis': {
            ...
            resolve: {
                        ...
                        $stateParams.orderableGroups = existingStockOrderableGroupsFactory
                            ...
                            .then(function(groups) {
                                return groups.filter(function(group) {
                                    return group[0].orderable.programs.find(function(program) {
                                        return program.programId === $stateParams.programToId;
                                    });
                                });
                            });
                    }
                        ...
                        return stockReasonsFactory.getAdjustmentReasons($stateParams.programId, facility.type.id)
                            .then(function(reasons) {
                                return reasons.filter(function(reason) {
                                    return reason.tags.includes('p2p');
                                });
                            });
                    }
                ...
                adjustmentType: function() {
                    return ADJUSTMENT_TYPE.PROGRAM_TRANSFER;
                },
                ...
                programTo: function($stateParams, programService) {
                    if (_.isUndefined($stateParams.programTo)) {
                        $stateParams.programTo = programService.get($stateParams.programToId);
                    }
                    return $stateParams.programTo;
                }
            }
        });
    }
